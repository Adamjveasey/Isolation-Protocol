...
public class RoomSpawner : MonoBehaviour
{
    ...
    // Interger used for random selection
    ...
    public void Spawn()
    {
        if ( !m_spawned )
        {
            ...
            {
                // If room limit has been reached in level controller spawns a dead end on this spawner instead
                switch ( m_doorDirection )
                {
                    case Enums.Directions.Top:
            ...
            // Adds the spawned room to the level controller room list
    ...
    private void OnTriggerEnter2D( Collider2D collision )
    {
        ...
        {
            // Upon collision with another spawn point that has not spawned a room either 

...
// Manages the saving and loading of data in the Ship Hub
public class SaveManager : MonoBehaviour
{
    [Tooltip("Reference to PlayerController")]
    public PlayerController               m_player;

    [Tooltip("Reference to drone fabricator upgrade list")]
    public FabricatorUpgradeListGenerator m_droneUpgrades;

    [Tooltip("Reference to exo suit fabricator upgrade list")]
    public FabricatorUpgradeListGenerator m_exoSuitUpgrades;

    // There were issues with loading the weapons due to a bug with the FabricatorUpgradeListGenerator array size,
    // so manual references to all weapon FabricatorStoreProduct were implemented
    [Tooltip("References to weapons store product objects")]
    public FabricatorStoreProduct[]       m_weaponUnlocks;

    [Tooltip("Reference to the weapon wall")]
    ...
    public void Load()
    {
        ...
        // Most of weapon equpping code taken from WeaponWall.cs so that it functions in the same way and doesn't feel different to the player
        // Changes were made to adapt it to loading in weapons by just their name
